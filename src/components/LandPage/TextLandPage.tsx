...
function textLandPage() {
        ...
        return (
          <div className="textContainer">
            ...
            <div>{textArray2.map((text, index) => (
              <a key={index} className={`text ${index >= 3 ? 'darkColor': 'lightColor' }`}>
                {text}
              </a>
            ))}</div>
           <div> {textArray3.map((text, index) => (
              <a key={index} className={`text ${index  >= 2 ? 'lightColor' : 'darkColor' }`}>
                {text}
              </a>
            ))}</div>
            </div>
